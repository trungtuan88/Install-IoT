...
CLONE_VIINDOO_DIR="${__dir}/viindoo_iot_hw"
VERSION_IOT=16.0
...
rm -rfv "${CLONE_VIINDOO_DIR}"

if [ ! -d $CLONE_VIINDOO_DIR ]; then
    echo "Clone Viindoo IoT Github repo"
    read -sp "Enter your GitHub token: " GITHUB_TOKEN
    echo
    REPO_IOT=https://${GITHUB_TOKEN}@github.com/Viindoo/iot-hw.git
    mkdir -pv "${CLONE_VIINDOO_DIR}"
    git clone -b ${VERSION_IOT} --no-local --no-checkout --depth 1 ${REPO_IOT} "${CLONE_VIINDOO_DIR}"
    cd "${CLONE_VIINDOO_DIR}"
    git config core.sparsecheckout true
    echo "viin_hw_drivers
viin_hw_homepage" | tee --append .git/info/sparse-checkout > /dev/null
    git read-tree -mu HEAD
fi
...
# Tệp cần chỉnh sửa
target_file="${OVERWRITE_FILES_BEFORE_INIT_DIR}"/home/pi/odoo/addons/point_of_sale/tools/posbox/overwrite_before_init/etc/init_posbox_image.sh

# Dòng cần tìm kiếm
search_line_PKGS_TO_INSTALL="xserver-xorg-video-fbdev"

search_line_PIP_TO_INSTALL="rjsmin==1.1.0"

# Dòng thay thế
replace_line_PKGS_TO_INSTALL='xserver-xorg-video-fbdev \\\
    nmap'

replace_line_PIP_TO_INSTALL='rjsmin==1.1.0 \\\
    num2words==0.5.9 \\\
    scapy \\\
    ipaddress \\\
    ngrok'
...
# Dòng sử dụng sed để tìm kiếm và thay thế văn bản
sed -i "s/$search_line_PKGS_TO_INSTALL/$replace_line_PKGS_TO_INSTALL/g" "$target_file"
sed -i "s/$search_line_PIP_TO_INSTALL/$replace_line_PIP_TO_INSTALL/g" "$target_file"
...
cp -av "${OVERWRITE_FILES_BEFORE_INIT_DIR}"/home/pi/odoo/addons/point_of_sale/tools/posbox/overwrite_before_init/* "${MOUNT_POINT}"
mkdir -pv "${OVERWRITE_FILES_AFTER_INIT_DIR}"
cp -av "${OVERWRITE_FILES_BEFORE_INIT_DIR}"/home/pi/odoo/addons/point_of_sale/tools/posbox/overwrite_after_init/* "${OVERWRITE_FILES_AFTER_INIT_DIR}"

cp -av "${CLONE_VIINDOO_DIR}"/* "${MOUNT_POINT}/home/pi/odoo/addons/"
